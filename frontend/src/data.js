export const compliments = [
  "Your smile lights up my entire world 💫",
  "You make ordinary moments feel extraordinary",
  "Your kindness touches everyone around you",
  "I love how you see beauty in everything",
  "Your laugh is my favorite sound in the world",
  "You have the most beautiful heart I've ever known",
  "The way you care for others is truly inspiring",
  "Your strength amazes me every single day",
  "You make me want to be a better person",
  "Your eyes sparkle with such warmth and love",
  "I admire your courage to be authentically you",
  "You bring so much joy into my life",
  "Your intelligence and wit never cease to amaze me",
  "You have a way of making everyone feel special",
  "I love your passion for the things you care about",
  "Your hugs feel like home to me",
  "You're the most thoughtful person I know",
  "Your creativity inspires me constantly",
  "I love how you never give up on your dreams",
  "You make even the hardest days feel manageable",
  "Your positive energy is absolutely contagious",
  "I'm so grateful for your patience and understanding",
  "You have the most adorable quirks that make you, you",
  "Your presence makes everything feel right",
  "I love how you find joy in the little things",
  "You're stronger than you know and braver than you believe",
  "Your compassion makes the world a better place",
  "I fall in love with you more every single day",
  "You're my favorite person to spend time with",
  "Thank you for being exactly who you are ❤️"
];

export const admirationPoints = [
  {
    icon: "Heart",
    title: "Your Kindness",
    description: "The way you show compassion to everyone around you, even when you're having a tough day yourself"
  },
  {
    icon: "Sparkles",
    title: "Your Creativity",
    description: "How you see beauty and possibility where others see ordinary, and turn ideas into magic"
  },
  {
    icon: "Star",
    title: "Your Strength",
    description: "The quiet resilience you show in facing challenges, always finding a way forward"
  },
  {
    icon: "Smile",
    title: "Your Laughter",
    description: "That genuine, contagious laugh that can brighten any room and lift any spirit"
  },
  {
    icon: "Book",
    title: "Your Curiosity",
    description: "The way you're always learning, growing, and eager to understand the world better"
  },
  {
    icon: "Sun",
    title: "Your Energy",
    description: "How you light up any space you enter with your warmth and positive presence"
  }
];

export const memories = [
  {
    image: "https://customer-assets.emergentagent.com/job_79c7bba3-d536-4831-b09b-c3c1c8e79b60/artifacts/qpygdnt4_WhatsApp%20Image%202026-01-21%20at%2016.31.15.jpeg",
    caption: "",
    rotation: -2
  },
  {
    image: "https://customer-assets.emergentagent.com/job_79c7bba3-d536-4831-b09b-c3c1c8e79b60/artifacts/ibank88n_WhatsApp%20Image%202026-01-21%20at%2016.31.16.jpeg",
    caption: "",
    rotation: 3
  },
  {
    image: "https://customer-assets.emergentagent.com/job_79c7bba3-d536-4831-b09b-c3c1c8e79b60/artifacts/uj65hzep_WhatsApp%20Image%202026-01-21%20at%2016.31.49%20%281%29.jpeg",
    caption: "",
    rotation: -1
  },
  {
    image: "https://customer-assets.emergentagent.com/job_79c7bba3-d536-4831-b09b-c3c1c8e79b60/artifacts/b3m5w6i7_WhatsApp%20Image%202026-01-21%20at%2016.31.49.jpeg",
    caption: "",
    rotation: 2
  }
];

export const quotes = [
  {
    text: "In a world full of temporary things, you are a perpetual feeling.",
    author: "Sanober Khan"
  },
  {
    text: "You are my today and all of my tomorrows.",
    author: "Leo Christopher"
  },
  {
    text: "I love you not only for what you are, but for what I am when I am with you.",
    author: "Roy Croft"
  },
  {
    text: "You are the finest, loveliest, tenderest person I have ever known.",
    author: "F. Scott Fitzgerald"
  }
];
